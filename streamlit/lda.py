  ...
  if uploaded_file is not None:
      ...
      with col2:
        ...
        if target_column is not None:
            n_components = st.slider(label="Number of dimensions to project into.", min_value=1, max_value=np.min([
                len(feature_names), len(dataframe[target_column].unique())-1]), value=1, step=1, disabled=False, label_visibility="visible")
